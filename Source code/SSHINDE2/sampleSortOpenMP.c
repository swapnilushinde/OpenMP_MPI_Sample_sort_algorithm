...
//
	...
	int p,n; // processorts_count, thread_count, element_count
	int *input;
...
void sort(int * localList,int n)
{
	...
	for(int i=0;i<n;i++)
	{
		key = *(localList+i);
		index = i;
		#pragma omp parallel shared(key,localList,n,index,i) private(j) num_threads(t)
		{
...
void countElements(int *localList, int target, int *count, int n)
{
	...
	//while(lo<=hi)
	//#pragma omp parallel default(private) shared(lo,hi,localhost,target,flag,count) private(temp,mid,)num_threads(t)
	#pragma omp parallel shared(lo,hi,localList,target,count,flag) private(temp,mid) num_threads(t)
	{
		#pragma omp for schedule(dynamic)
			for(lo=0;lo<=hi;lo++)
			{
				if(!flag)
				{
					...
					//just added to satisfy increment condition in for loop.	
					lo = lo - 1;	
				}	
			}
	}
...
void merge(int * localList, int * rdispls, int n,int p, int * sortedBucket)
{
		...
		//#pragma omp parallel shared(sentinel,k,p,rdispls,localList,sortedBucket) private(i,min) firstprivate(minIndex) num_threads(t)		
		
		#pragma omp parallel shared(sentinel,p,n,rdispls,localList,sortedBucket) private(i,k,min,minIndex) num_threads(t)		
		{
			minIndex = 0;
			
			#pragma omp for schedule(dynamic) ordered
				for (k=0 ;k<n;k++) //localList
				{
					#pragma omp ordered
					{
